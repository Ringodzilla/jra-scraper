...
from .validation import OUTPUT_COLUMNS, validate_rows
...
class JRAPipeline:
    ...
    def run(
        ...
        *,
        ...
        logger.info(
            "Pipeline start race_limit=%s horse_limit=%s reprocess_raw=%s force_rebuild=%s",
            race_limit,
            horse_limit,
            reprocess_raw,
            force_rebuild,
        ...
        for race in races:
            ...
            horses = self.parser.parse_race_detail(race_html, race.race_id, race.race_name)[:horse_limit]
